...
void* guardar_SWAP(void* proceso){
    // ESCRIBIR EN EL FINAL DEL ARCHIVO Y USAR SEMAFORO
    return;
}

void* leer_SWAP(){
    struct pcb p;
    // cargar EN EL FINAL DEL ARCHIVO
    return;
}
...
void leer_pseudo(){
    if( (FILE_PSEUDOCODIGO=fopen(DIR_PSEUDOCODIGO,"r")) !=NULL){ // VER COMO ENVIAR MENOS DE 255
        char cont [255];
        while (fgets(cont,255,FILE_PSEUDOCODIGO)){
            char * guarda = malloc(sizeof(char) * 256);
            guarda=strcat(cont,"\0");
...
void enviar_lista(){
    ...
    {
        printf(list_remove(lista_procesos,0)); // NO LO MUESTRA POR LA CONSOLA 
    }
    return;
}
